...
namespace Application.Services.Implementations
{
    public class CustomerService : ICustomerService
    {
        ...
        public async Task<CustomerDto> CreateAsync(CustomerCreateDto request)
        {
            ...
            customer.CreatedBy = _authService.GetSignedInEmail() ?? "System";
        ...
        public async Task<CustomerDto?> GetByEmailAsync(string email)
        {
            ...
            if (_authService.IsCustomer() && signedInEmail != customer.Email)
            {
                _logger.LogWarning("Unauthorized access attempt to get customer details with Email: {Email} by email: {SignedInEmail}", email, signedInEmail);
                throw new UnauthorizedAccessException("You are not authorized to access this customer's information.");
            }
        ...
        public async Task<CustomerDto?> UpdateAsync(Guid id, CustomerUpdateRequest updateRequest)
        {
            ...
            {
                var (hash, salt) = UserHelper.GeneratePasswordHash(updateRequest.Password);
                customer.PasswordHash = hash;
                customer.HashSalt = salt;
            }

            if (updateRequest.Gender.HasValue)
            {
                customer.Gender = updateRequest.Gender.Value;
            }

            customer.UpdatedBy = _authService.GetSignedInEmail() ?? "System";
